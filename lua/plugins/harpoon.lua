return {
	...
	config = function()
		...
		map('n', '<M-l>', function() harpoon.ui:toggle_quick_menu(harpoon:list()) end,
			{ desc = 'harpoon [L]ist' })
		map('n', '<D-l>', function() harpoon.ui:toggle_quick_menu(harpoon:list()) end,
			{ desc = 'harpoon [L]ist' })
		...
		for _, idx in ipairs { 1, 2, 3, 4, 5, 6 } do
			map('n', string.format('<leader>%d', idx), function() harpoon:list():select(idx) end,
		...
		for _, idx in ipairs { 1, 2, 3, 4, 5 } do
			map('n', string.format('<localleader>hd%d', idx), function() delete_at_index(idx) end,
				{ desc = string.format('harpoon delete %d', idx) })
		end
		...
		for _, idx in ipairs { 1, 2, 3, 4, 5 } do
			map('n', string.format('<localleader>h%d', idx), function() add_at_index(idx) end,
				{ desc = string.format('harpoon add at index%d', idx) })
		end
